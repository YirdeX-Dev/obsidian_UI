...
local BaseURL = "https://raw.githubusercontent.com/SyndromeXph/Obsidian/refs/heads/main/"
...
function Library:SetBackgroundImageEnabled(State: boolean)
    ...
    if self.Window and self.Window.BackgroundImage then
...
function Library:SetBackgroundImage(Image: string | number)
    ...
    local CustomImage = Library:GetCustomImage(Image)
    self.Scheme.BackgroundImage = CustomImage.Url
    if self.Window and self.Window.BackgroundImage then
        self.Window.BackgroundImage.Image = CustomImage.Url
    end
    self:UpdateColorsUsingRegistry()
end
...
local BackgroundImage = New("ImageLabel", {
    Image = Library.Scheme.BackgroundImage,
    ...
    Visible = Library.Scheme.BackgroundImageEnabled,
    Parent = MainFrame,
})
...
local Window = {
    ...
    if self.Glow then
        self.Glow.Visible = State
    end
    Library:UpdateColorsUsingRegistry()
end


function Window:SetBackgroundImageEnabled(State)
    assert(typeof(State) == "boolean", "Expected boolean for State, got: " .. typeof(State))
    
    Library.Scheme.BackgroundImageEnabled = State
    if self.BackgroundImage then
        self.BackgroundImage.Visible = State
    end
    Library:UpdateColorsUsingRegistry()
end
...
function Window:SetBackgroundImage(Image)
    assert(typeof(Image) == "string" or typeof(Image) == "number", 
        "Expected string/number for Image, got: " .. typeof(Image))
    
    local CustomImage = Library:GetCustomImage(Image)
    Library.Scheme.BackgroundImage = CustomImage.Url
    if self.BackgroundImage then
        self.BackgroundImage.Image = CustomImage.Url
    end
    Library:UpdateColorsUsingRegistry()
end
